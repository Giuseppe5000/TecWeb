...
L’implementazione del \textit{Responsive Web Design} (RWD) ha permesso di adattare dinamicamente il layout del sito per diversi dispositivi e risoluzioni. Abbiamo utilizzato una struttura modulare basata su tre file CSS:
\begin{itemize}
\item \textbf{style.css:} Contiene le regole principali, pensate per dispositivi mobili e successivamente estese a risoluzioni maggiori;
\item \textbf{mini.css:} Adatta ulteriormente lo stile per schermi di dimensioni particolarmente ridotte;
\item \textbf{print.css:} Ottimizza la resa grafica per la stampa.
\end{itemize}
